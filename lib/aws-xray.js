...
(function () {
  ...
  var sdkData = {
    ...
    sdk_version: pkginfo.version || UNKNOWN,
  ...
  var contextMissing = process.env.AWS_XRAY_CONTEXT_MISSING;
  var lookupStrategy = contextMissing ? contextUtils.lookupContextMissingStrategy(contextMissing) : null;
  if (lookupStrategy) {
    logging.getLogger().info('Environment variable with key AWS_XRAY_CONTEXT_MISSING is set. Updating contextMissingStrategy.contextMissing: ' + contextMissing + '.');
    contextUtils.contextMissingStrategy = lookupStrategy;
  }
